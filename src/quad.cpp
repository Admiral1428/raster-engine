...
void Quad::make_shape()
{
    ...
    // mid, p1, p2
    first = Surface(Eigen::Vector4f(pmid[0], pmid[1], pmid[2], 1.0f),
                    Eigen::Vector4f(p1[0], p1[1], p1[2], 1.0f),
                    Eigen::Vector4f(p2[0], p2[1], p2[2], 1.0f), colors[0], diminish_light);

    // mid, p2, p3
    second = Surface(Eigen::Vector4f(pmid[0], pmid[1], pmid[2], 1.0f),
                     Eigen::Vector4f(p2[0], p2[1], p2[2], 1.0f),
                     Eigen::Vector4f(p3[0], p3[1], p3[2], 1.0f), colors[0], diminish_light);

    // mid, p3, p0
    third = Surface(Eigen::Vector4f(pmid[0], pmid[1], pmid[2], 1.0f),
                    Eigen::Vector4f(p3[0], p3[1], p3[2], 1.0f),
                    Eigen::Vector4f(p0[0], p0[1], p0[2], 1.0f), colors[0], diminish_light);

    // mid, p0, p1
    fourth = Surface(Eigen::Vector4f(pmid[0], pmid[1], pmid[2], 1.0f),
                     Eigen::Vector4f(p0[0], p0[1], p0[2], 1.0f),
                     Eigen::Vector4f(p1[0], p1[1], p1[2], 1.0f), colors[0], diminish_light);
